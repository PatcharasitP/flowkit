...
{
  const m = g("เริ่มงาน\nตรวจเอกสาร (( ต้องมีสำเนาบัตร ปชช. ))\nครบไหม? ((ดูให้ครบ 3 อย่าง))\n  ได้: ปิดงาน ((ปิดในระบบด้วย))\n  ไม่ได้: [บัญชี] ส่งกลับ ((แนบเหตุผล))");
  const tip = (t) => (m.nodes.find((n) => n.text === t) || {}).tip;
  ck(tip("ตรวจเอกสาร") === "ต้องมีสำเนาบัตร ปชช." && !m.nodes.some((n) => n.text.includes("((")), "ท้ายบรรทัด (( )) = ป้าย ข้อความในกล่องไม่มีวงเล็บ", JSON.stringify(m.nodes.map((n) => [n.text, n.tip])));
  ck(m.nodes.find((n) => n.text === "ครบไหม?").shape === "ask" && tip("ครบไหม?") === "ดูให้ครบ 3 อย่าง", "คำถามที่มีป้ายยังเป็นคำถาม (? ไม่ถูกบังด้วยวงเล็บ)");
  ck(tip("ปิดงาน") === "ปิดในระบบด้วย" && tip("ส่งกลับ") === "แนบเหตุผล", "ป้ายติดกล่องหลัง คำตอบ: และหลัง [ฝ่าย]");
  ck(!tip("เริ่มงาน") && m.warnings.length === 0, "กล่องที่ไม่มีป้าย ไม่มี tip และไม่มีคำเตือน");
  const trap = g("เริ่ม ((ยังไม่ปิด\nทำ (กลม) งาน\nกลาง ((ไม่ใช่ท้าย)) บรรทัด\nจบงานนี้");
  ck(trap.nodes.map((n) => n.text).join("|") === "เริ่ม ((ยังไม่ปิด|ทำ (กลม) งาน|กลาง ((ไม่ใช่ท้าย)) บรรทัด|จบงานนี้" && !trap.nodes.some((n) => n.tip),
    "กับดัก: (( ไม่ปิด , วงเล็บชั้นเดียว , (( )) กลางบรรทัด เป็นข้อความธรรมดาทั้งหมด", trap.nodes.map((n) => n.text).join("|"));
  const sys = g("ผัง: ระบบ\nExcel -> Power BI: โหลดข้อมูล ((ทุกเช้า))");
  ck(sys.warnings.some((w) => w.line === 2 && /\(\( \)\)/.test(w.text)) && !sys.nodes.some((n) => n.tip), "บรรทัดที่ไม่ใช่กล่อง (เส้นของผังระบบ) เตือนพร้อมเลขบรรทัด ไม่เดาใส่กล่องไหน");
}
